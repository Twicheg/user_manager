...
class Users(Base):
    ...
    name: Mapped[Optional[str]] = mapped_column(max_lenght=150,unique=True)
    country: Mapped[Optional[List["Countries"]]] = relationship(
        back_populates="user", cascade="all, delete-orphan")

    def __repr__(self) -> str:
        return f"User(id={self.counter}, name={self.name}, country={self.country})"
...
class Countries(Base):
    ...
    country_id: Mapped[str] = mapped_column(max_lenght=10)
