...
async def get_user_from_request(user_name, url):
...
async def add_to_database(user, session):
    try:
        if user.__class__ is dict:
            user = Users(name=user.get("name"), country=[Countries(**i) for i in user.get("country")])
        else:
